  ...
  <v-container v-else fluid :class="$vuetify.breakpoint.xs ? 'pa-0' : ''">
    <v-card outlined :tile="$vuetify.breakpoint.xs">
      <v-toolbar color="transparent" dense flat>
        <v-toolbar-title>
          <v-btn color="info" depressed small class="mr-4" @click="goCategory">
            {{ article.category }}
      ...
      <v-divider />
      <v-card-title>
        <v-icon color="error" left v-if="newCheck(article.updatedAt)"
          >mdi-fire</v-icon
        >
      ...
      <v-card-actions>
        ...
        <v-sheet class="mr-0">
          <v-icon left :color="article.commentCount ? 'info' : ''"
            >mdi-comment</v-icon
          >
